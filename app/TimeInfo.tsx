...
const TimeInfo = () => {
    const [timeText, setTimeText] = useState(moment().format('M월 D일 • H시 m분'))
    ...
    return (
        <div className="p-6   rounded-xl space-y-4 bg-white">
            ...
            <h2 className="text-2xl font-bold flex justify-between"><span>자율학습 <button onClick={() => period == 0 && setProgress(p => p + 1)}>{period}</button>교시  </span> <span className='text-gray-500 block font-normal text-sm'>
                {period == 0 && '현재는 자율학습 운영시간이 아닙니다요'}
            </span>  </h2>


            <div>
                <Progress value={progress} className="w-full bg-indigo-100 h-3 rounded-md " />
